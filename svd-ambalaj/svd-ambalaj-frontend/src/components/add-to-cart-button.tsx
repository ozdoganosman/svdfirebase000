...
export function AddToCartButton({
  ...
  // Compact mode for product cards - just an icon button
  if (compact) {
    return (
      <button
        type="button"
        ...
        disabled={isDisabled}
        className={`flex-1 inline-flex items-center justify-center gap-1.5 rounded-lg bg-amber-500 px-3 py-1.5 text-xs font-medium text-white transition hover:bg-amber-600 disabled:opacity-50 disabled:cursor-not-allowed ${className}`.trim()}
      >
        {isPending ? (
