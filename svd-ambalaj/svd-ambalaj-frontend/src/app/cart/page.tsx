...
export default function CartPage() {
  ...
  const handleExportPDF = async () => {
    ...
    // KDV: hem ürünler hem kargo üzerinden hesaplanır
    const kdvTutari = (kdvHaricTutar + kargoUcreti) * kdvOrani;
    const genelToplam = kdvHaricTutar + kargoUcreti + kdvTutari;
        ...
        <div className="mt-12 grid gap-8 lg:grid-cols-[2fr_1fr]">
          <div className="space-y-6">
            ...
            {items.map((item) => {
              ...
              // Calculate savings for bulk discounts
                ...
                <div className="space-y-3">
                  ...
                  {nextTier && exchangeRate && (
                    <div className="rounded-lg bg-amber-50 px-3 py-2 text-xs text-amber-700">
                      ...
                      birim fiyat {formatDualPrice(undefined, exchangeRate, true, 1, nextTier.price * exchangeRate)} +KDV olsun!
                    </div>
                  )}
            ...
            <aside className="rounded-2xl border border-amber-100 bg-amber-50 p-6 shadow-sm">
              ...
              <dl className="mt-6 space-y-3 text-sm">
                ...
                {comboDiscount > 0 && (
                  <div className="flex items-center justify-between text-slate-500">
                    <dt>Ürün Toplamı</dt>
                    <dd className="line-through">{exchangeRate ? formatDualPrice(undefined, exchangeRate, true, 1, subtotal) : "₺" + subtotal.toLocaleString("tr-TR")} <span className="text-xs font-normal">+KDV</span></dd>
                  </div>
                )}
                ...
                <div className="flex items-center justify-between text-slate-700 font-semibold">
                  ...
                  <dd className="text-amber-700">{exchangeRate ? formatDualPrice(undefined, exchangeRate, true, 1, subtotal - comboDiscount) : "₺" + (subtotal - comboDiscount).toLocaleString("tr-TR")} <span className="text-xs font-normal text-slate-500">+KDV</span></dd>
                </div>
                ...
                <div className="flex items-center justify-between text-slate-500 pt-2 border-t border-amber-100">
                  <dt>KDV (%{taxRate})</dt>
                  <dd>{exchangeRate ? formatDualPrice(undefined, exchangeRate, true, 1, ((subtotal - comboDiscount) + (totalItems >= 50000 ? 0 : totalBoxes * 120)) * (taxRate / 100)) : "₺" + (((subtotal - comboDiscount) + (totalItems >= 50000 ? 0 : totalBoxes * 120)) * (taxRate / 100)).toLocaleString("tr-TR")}</dd>
                </div>
              </dl>
              <div className="mt-6 border-t border-amber-100 pt-4">
                <div className="flex items-center justify-between text-base font-bold text-amber-700">
                  ...
                  <span>{exchangeRate ? formatDualPrice(undefined, exchangeRate, true, 1, ((subtotal - comboDiscount) + (totalItems >= 50000 ? 0 : totalBoxes * 120)) * (1 + taxRate / 100)) : "₺" + (((subtotal - comboDiscount) + (totalItems >= 50000 ? 0 : totalBoxes * 120)) * (1 + taxRate / 100)).toLocaleString("tr-TR")}</span>
                </div>
                <p className="mt-2 text-xs text-slate-600">
                  KDV hariç: {exchangeRate ? formatDualPrice(undefined, exchangeRate, true, 1, (subtotal - comboDiscount) + (totalItems >= 50000 ? 0 : totalBoxes * 120)) : "₺" + ((subtotal - comboDiscount) + (totalItems >= 50000 ? 0 : totalBoxes * 120)).toLocaleString("tr-TR")} +KDV
                </p>
              </div>
              ...
              <div className="mt-6 space-y-2">
                ...
                {user ? (
                  ...
                  <Link
                    href="/auth/login?redirect=/cart&action=sample"
                    className="flex w-full items-center justify-center gap-2 rounded-xl border-2 border-blue-200 bg-blue-50 px-4 py-2.5 text-sm font-semibold text-blue-700 transition hover:border-blue-300 hover:bg-blue-100"
                  >
                    <svg className="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2" />
                    </svg>
                    Numune Talebi (Giriş Yapın)
                  </Link>
                )}
                {user ? (
                  <button
                  ...
                  <Link
                    href="/auth/login?redirect=/cart&action=quote"
                    className="flex w-full items-center justify-center gap-2 rounded-xl border-2 border-purple-200 bg-purple-50 px-4 py-2.5 text-sm font-semibold text-purple-700 transition hover:border-purple-300 hover:bg-purple-100"
                  >
                    <svg className="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                    </svg>
                    Teklif Al (Giriş Yapın)
                  </Link>
                )}
                {user ? (
                  <Link
                  ...
                  >
                    ...
                    Alışverişi Tamamla (Giriş Yapın)
                  </Link>
                )}
              </div>
            </aside>
